-- Require twenty guided position votes on voting-cycle visits:
-- 3 QB, 7 RB, 7 WR and 3 TE. Visit frequency is intentionally managed in
-- the browser; this function remains the authoritative per-session progress.
...
-- Keep the legacy RPC consistent for older cached clients.
...
begin
  ...
  if v_qb < 3 or v_rb < 7 or v_wr < 7 or v_te < 3 then
    raise exception 'complete 3 QB, 7 RB, 7 WR and 3 TE votes before unlocking';
  end if;

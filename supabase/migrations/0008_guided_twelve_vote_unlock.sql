-- Guide new visitors through two votes at every real fantasy position.
-- Previously unlocked sessions remain unlocked through their token_spends row.
...
as $$
  ...
  select
    ...
    least(c.qb_votes, 2)
      ...
      + least(c.te_votes, 2)
      + least(c.k_votes, 2)
      + least(c.dst_votes, 2) as qualified_votes,
...
begin
  ...
  into v_earned, v_qb, v_rb, v_wr, v_te, v_k, v_dst
  from public.votes
  where session_id = p_session_id;
  ...
  if v_qb < 2 or v_rb < 2 or v_wr < 2 or v_te < 2 or v_k < 2 or v_dst < 2 then
    raise exception 'complete the 12 guided position votes before unlocking';
  end if;
